...
namespace quat {
    // Quaternion: [x, y, z, w], same as https://glmatrix.net/docs/quat.js.html

    export function setIdentity(q: Fx8[]) {
        q[0] = q[1] = q[2] = Fx.zeroFx8
        q[3] = Fx.oneFx8
    }

    export function fromAxisAngleTwos(q: Fx8[], angleTwos: Fx8, ax: Fx8, ay: Fx8, az: Fx8) {
        const halfAngle = Fx.rightShift(angleTwos, 1)
        const s = Fx.sinFx8(halfAngle)
        q[0] = Fx.mul(s, ax)
        q[1] = Fx.mul(s, ay)
        q[2] = Fx.mul(s, az)
        q[3] = Fx.cosFx8(halfAngle)
    }

    // Use this only for not-too-large magnitude, it overflows.
    export function lenSquared(q: Fx8[]): Fx8 {
        return Fx.multiplyAdd4(q[0], q[0], q[1], q[1], q[2], q[2], q[3], q[3])
    }

    export function normalizeSlowly(q: Fx8[]) {
        // This uses floating-point square root. Use sparingly.
        // TODO: do all calculations in float for accuracy?
        const len = Fx8(Math.sqrt(Fx.toFloat(lenSquared(q))))
        q[0] = Fx.div(q[0], len)
        q[1] = Fx.div(q[1], len)
        q[2] = Fx.div(q[2], len)
        q[3] = Fx.div(q[3], len)
    ...
    export function mul(out: Fx8[], a: Fx8[], b: Fx8[]) {
        out[0] = Fx.multiplyAdd4(a[0], b[3], a[3], b[0], a[1], b[2], Fx.neg(a[2]), b[1])
        out[1] = Fx.multiplyAdd4(a[1], b[3], a[3], b[1], a[2], b[0], Fx.neg(a[0]), b[2])
        out[2] = Fx.multiplyAdd4(a[2], b[3], a[3], b[2], a[0], b[1], Fx.neg(a[1]), b[0])
        out[3] = Fx.multiplyAdd4(a[3], b[3], Fx.neg(a[0]), b[0], Fx.neg(a[1]), b[1], Fx.neg(a[2]), b[2])
    }

    const tmpQ = [Fx.zeroFx8, Fx.zeroFx8, Fx.zeroFx8, Fx.oneFx8]

    export function postMul(out: Fx8[], b: Fx8[]) {
        const a = tmpQ
        a[0] = out[0]
        a[1] = out[1]
        a[2] = out[2]
    ...
    export function preMul(out: Fx8[], a: Fx8[]) {
        const b = tmpQ
        b[0] = out[0]
        b[1] = out[1]
        b[2] = out[2]
    ...
    export function rotateX(q: Fx8[], halfAngleTwos: Fx8) {
        const s = Fx.sinFx8(halfAngleTwos)
        const c = Fx.cosFx8(halfAngleTwos)
        const x = q[0]
        const y = q[1]
        const z = q[2]
        const w = q[3]
        q[0] = Fx.multiplyAdd2(x, c, w, s)
        q[1] = Fx.multiplyAdd2(y, c, z, s)
        q[2] = Fx.multiplyAdd2(z, c, Fx.neg(y), s)
        q[3] = Fx.multiplyAdd2(w, c, Fx.neg(x), s)
    }

    export function rotateY(q: Fx8[], halfAngleTwos: Fx8) {
        const s = Fx.sinFx8(halfAngleTwos)
        const c = Fx.cosFx8(halfAngleTwos)
        const x = q[0]
        const y = q[1]
        const z = q[2]
        const w = q[3]
        q[0] = Fx.multiplyAdd2(x, c, Fx.neg(z), s)
        q[1] = Fx.multiplyAdd2(y, c, w, s)
        q[2] = Fx.multiplyAdd2(z, c, x, s)
        q[3] = Fx.multiplyAdd2(w, c, Fx.neg(y), s)
    }

    export function rotateZ(q: Fx8[], halfAngleTwos: Fx8) {
        const s = Fx.sinFx8(halfAngleTwos)
        const c = Fx.cosFx8(halfAngleTwos)
        const x = q[0]
        const y = q[1]
        const z = q[2]
        const w = q[3]
        q[0] = Fx.multiplyAdd2(x, c, y, s)
        q[1] = Fx.multiplyAdd2(y, c, Fx.neg(x), s)
        q[2] = Fx.multiplyAdd2(z, c, w, s)
        q[3] = Fx.multiplyAdd2(w, c, Fx.neg(z), s)
    }

    export function setMat33(out: Fx8[], q: Fx8[]) {
        const lenSq = lenSquared(q)
        // TODO: use a multiplier for invLenSq to get a bit more accuracy?
        const invLenSq = Fx.div(Fx.oneFx8, lenSq)

        const x = q[0]
        const y = q[1]
        const z = q[2]
        const w = q[3]
        const x2 = Fx.leftShift(x, 1)
        const y2 = Fx.leftShift(y, 1)
        const z2 = Fx.leftShift(z, 1)
        const xx = Fx.mul(Fx.mul(x, x2), invLenSq)
        const yx = Fx.mul(Fx.mul(y, x2), invLenSq)
        const yy = Fx.mul(Fx.mul(y, y2), invLenSq)
        const zx = Fx.mul(Fx.mul(z, x2), invLenSq)
        const zy = Fx.mul(Fx.mul(z, y2), invLenSq)
        const zz = Fx.mul(Fx.mul(z, z2), invLenSq)
        const wx = Fx.mul(Fx.mul(w, x2), invLenSq)
        const wy = Fx.mul(Fx.mul(w, y2), invLenSq)
        const wz = Fx.mul(Fx.mul(w, z2), invLenSq)

        out[0] = Fx.sub(Fx.oneFx8, Fx.add(yy, zz))
        out[1] = Fx.add(yx, wz)
        out[2] = Fx.sub(zx, wy)

        out[3] = Fx.sub(yx, wz)
        out[4] = Fx.sub(Fx.oneFx8, Fx.add(xx, zz))
        out[5] = Fx.add(zy, wx)

        out[6] = Fx.add(zx, wy)
        out[7] = Fx.sub(zy, wx)
        out[8] = Fx.sub(Fx.oneFx8, Fx.add(xx, yy))
    }
}
...
namespace Fx {
    export function sgn(a: Fx8) {
        const x = a as any as number
        if (x == 0) return 0
        return x < 0 ? -1 : 1
    }
    ...
    export function multiplyAdd2(a0: Fx8, b0: Fx8, a1: Fx8, b1: Fx8): Fx8 {
        return (Math.imul(a0 as any as number, b0 as any as number) +
                Math.imul(a1 as any as number, b1 as any as number) >> 8) as any as Fx8
    }
    ...
    export function multiplyAdd3Plus1(a0: Fx8, b0: Fx8, a1: Fx8, b1: Fx8, a2: Fx8, b2: Fx8, c: Fx8): Fx8 {
        return ((Math.imul(a0 as any as number, b0 as any as number) +
                 Math.imul(a1 as any as number, b1 as any as number) +
                 Math.imul(a2 as any as number, b2 as any as number) >> 8) + (c as any as number)) as any as Fx8
    }

    export function multiplyAdd4(a0: Fx8, b0: Fx8, a1: Fx8, b1: Fx8, a2: Fx8, b2: Fx8, a3: Fx8, b3: Fx8): Fx8 {
        return (Math.imul(a0 as any as number, b0 as any as number) +
                Math.imul(a1 as any as number, b1 as any as number) +
                Math.imul(a2 as any as number, b2 as any as number) +
                Math.imul(a3 as any as number, b3 as any as number) >> 8) as any as Fx8
    }
}
...
class Renderer3d {
    ...
    */
    ...
    setPalette8Gray8Color() {
        const palBuf: Buffer = hex`000000333333555555777777999999bbbbbbddddddffffff004499eeee0000cc00dd0000eeeebb558800ffbb0000bbff`
    ...
    setViewerPoseFx8(poseMatrix: Fx8[]) {
        setInverseTransformFP(this.viewerFromWorld as any[] as Fx8[], poseMatrix)
    }
    ...
    drawFrame(image: Image) {
        ...
        for (let x = 0; x < 160; ++x) {
            ...
            */
            ...
            // At the horizontal screen center, keep track if we draw anything
            // at the screen center. That helps confirm if a laser shot hit the target.
            if (x == 80) {
                this.drawnAtCenter = false
                for (let l = 0; l < active.length; ++l) {
                    const tri = active[l]
                    const y0 = Fx.toIntFloor(tri.a_y)
                    const y1 = Fx.toIntFloor(tri.b_y)
                    if (y0 <= 60 && y1 >= 60) {
                        this.drawnAtCenter = true
                        break
                    }
                }
            }
...
function setInverseTransformFP(o: Fx8[], m: Fx8[]) {
    // Inverse of a rotation + translation matrix, converted to fixed point.
    // Transpose the translation part, and apply the transposed (inverse)
    // rotation to the original translation followed by negating it.
    o[0] = m[0]
    ...
    o[7] = m[5]
    o[8] = m[8]
    o[9] = Fx.multiplyAdd3(Fx.neg(o[0]), m[9],
                           Fx.neg(o[3]), m[10],
                           Fx.neg(o[6]), m[11])
    o[10] = Fx.multiplyAdd3(Fx.neg(o[1]), m[9],
                            Fx.neg(o[4]), m[10],
                            Fx.neg(o[7]), m[11])
    o[11] = Fx.multiplyAdd3(Fx.neg(o[2]), m[9],
                            Fx.neg(o[5]), m[10],
                            Fx.neg(o[8]), m[11])
}
...
function normalizeFP(v: number[]) {
    ...
    // Round towards zero to ensure vectors with negative components also have length <= 1
    v[0] = Math.trunc(v[0] * FP_ONE / len)
    v[1] = Math.trunc(v[1] * FP_ONE / len)
    v[2] = Math.trunc(v[2] * FP_ONE / len)
}
...
// FIXME:
function lenSq(vec: Fx8[]) : Fx8 {
    const x = vec[0] as any as number >> 4
    const y = vec[1] as any as number >> 4
    const z = vec[2] as any as number >> 4
    return Math.imul(x, x) + Math.imul(y, y) + Math.imul(z, z) as any as Fx8
}
/*
function debugLenSq(v: number[]) {
    const vec = [Fx8(v[0]), Fx8(v[1]), Fx8(v[2])]
    console.log('vec=' + v.join(', ') + ' lenSq=' + Fx.toFloat(lenSq(vec)) + ' dot_FP=' + dot_FP(vec as any as number[], vec as any as number[]) / 256)
}
debugLenSq([10, 20, 30])
debugLenSq([100, 200, 300])
debugLenSq([1000, 2000, 3000])
throw("quit")
*/
...
function mul_mat43_mat43_FP(outFP: number[], aFP: number[], bFP: number[]) {
    ...
    */
    ...
    const a = (aFP as any as Fx8[])
    const b = (bFP as any as Fx8[])
    out[ 9] = Fx.multiplyAdd3Plus1(a[0], b[9], a[3], b[10], a[6], b[11], a[ 9])
    out[10] = Fx.multiplyAdd3Plus1(a[1], b[9], a[4], b[10], a[7], b[11], a[10])
    out[11] = Fx.multiplyAdd3Plus1(a[2], b[9], a[5], b[10], a[8], b[11], a[11])
}
...
function mul_mat43_vec_FP(outFP: number[], mFP: number[], vFP: number[]) {
    ...
    */
    out[0] = Fx.multiplyAdd3Plus1(m[0], v[0], m[3], v[1], m[6], v[2], m[ 9])
    out[1] = Fx.multiplyAdd3Plus1(m[1], v[0], m[4], v[1], m[7], v[2], m[10])
    out[2] = Fx.multiplyAdd3Plus1(m[2], v[0], m[5], v[1], m[8], v[2], m[11])
}
...
namespace Fx {
    ...
    // The angleTwos argument is the angle in radians divided by Pi.
    // In other words, a full circle is 0..2 instead of 0..2*Pi.
    export function sinFx8(angleTwos: Fx8): Fx8 {
        ...
        // The result is exact within the 8-bit Fx8 precision. Note that there's no
        // point in having a finer angle precision - sin(x) is approx x for small
        // values, and adding intermediate angle values wouldn't generate any new
        // distinct values.
    ...
    /*
    let fakePi = 256
    for (let i = 0; i <= fakePi / 2; ++i) {
        const v = sinFx8(i as any as Fx8) as any as number
        const s = Math.round(256 * Math.sin(i * Math.PI / fakePi))
        console.log("sinFx8(" + i + ")=" + sinFx8(i as any as Fx8) + " sin=" + s + " error=" + (v - s))
    }

    console.log("sin(45)=" + Fx.toFloat(sinFx8(fakePi / 8 as any as Fx8)))
    console.log("cos(45)=" + Fx.toFloat(cosFx8(fakePi / 8 as any as Fx8)))
    console.log("sin(90)=" + Fx.toFloat(sinFx8(fakePi / 4 as any as Fx8)))
    console.log("cos(90)=" + Fx.toFloat(cosFx8(fakePi / 4 as any as Fx8)))
...
class InstanceBase {
    ...
    getPosX() : Fx8 {   
        return this.worldFromModel[9] as any as Fx8
    }
    getPosY() : Fx8 {
        return this.worldFromModel[10] as any as Fx8
    }
    getPosZ() : Fx8 {
        return this.worldFromModel[11] as any as Fx8
    }
    setPosX(x: Fx8) {
        this.worldFromModel[9] = x as any as number
    }
    setPosY(y: Fx8) {
        this.worldFromModel[10] = y as any as number
    }
    setPosZ(z: Fx8) {
        this.worldFromModel[11] = z as any as number
    }
...
namespace shader3d {
    export function getFlatColorShader(color: number) {
        return (unused_normal: number[], unused_z: number) => {
            return color
        }
    }
    ...
    export function getDiffuseShader(light: number[], col0: number, col1: number) {
        return (normal: number[], unused_z: number) => {
            ...
            // Careful, dot product may be slightly outside the -1..1 range due to rounding errors.

            // Regular shading: diffuse + ambient. Downside is that the unlit
            // side is a single color with no details.
            return Math.clamp(col0, col1, col0 + (dot > 0 ? (dot * (col1 - col0) >> FP_BITS) : 0))
        }
    }
    ...
    export function getHalfAngleDiffuseShader(light: number[], col0: number, col1: number) {
        return (normal: number[], unused_z: number) => {
            ...
            return Math.clamp(col0, col1, col0 + ((dot + FP_ONE) * (col1 - col0) >> FP_BITS >> 1))
        }
    }
    ...
    export function applyDistanceDimming(baseShader: Function, maxDistFP: number, col0: number) {
        ...
        return (normal: number[], z: number) => {
            ...
            // Alternative: linear brightness drop (seems a bit too dark)
...
class MeshModelBase {
    ...
    constructor() {
        ...
        this.boundingSphereRadiusSquared = 0 // unknown, override in instance
        ...
        */
        ...
        for (let i = 0; i < clippedFaces.length; ++i) {
            ...
            if (np == 3) {
                ...
                let color: number
                const material = this.faceMaterials[origIndex]
                if (material !== undefined) {
                    // FIXME: proper material handling
                    color = material
                } else {
                    color = Math.floor(shader(this.faceNormals_W[origIndex], -instance.viewerFromModel[11], origIndex))
                }
    ...
    constructor(model: MeshTreeModelBase, splitFace: number[], faces: number[][]) {
        ...
        if (faces) {
            ...
            for (let i = 0; i < faces.length; ++i) {
                ...
                const face = faces[i]
                if (face[0] < 0) {
                    model.faceMaterials[model.faces.length] = -face.shift()
                }
...
class IcosahedronModel extends MeshModelBase {
    ...
    constructor() {
        ...
        this.boundingSphereRadiusSquared = this.boundingSphereRadius * this.boundingSphereRadius >> FP_BITS
